...
类比：**context7 之于文档 → awsomeskills 之于 skill**——一个 agent 能按需撞上来、查询、拉取的发现层；
...
**awsomeskills 交付的是"信任信号"，不是"执行环境"。**
...
- 可复用（公开）：`article-pivot`（canonical + 处理适配器内核）、`catalog.toml` schema 范式、`REDACTED-PRIVATE-REPO` 里 grounding 的**公开 SKILL 部分**。
