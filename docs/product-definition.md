...
- [ ] **digest 清单不完整 → verify 被拒**：`files_complete: false` 的条目会被 `verify_skill.py` 拒绝
      （这是设计如此：不完整的清单不能冒充校验通过）。2026-08-31 两轮修复：可执行文件上限 50 → 200
      （实测最大 134），该成因归零；tree inventory 截断改为**按选中的 skill 目录做子树遍历**，
      巨型仓不再整体放弃。**注意两个完整度标志不可混用**：`bundle_complete`（选中 skill 的文件是否齐全，
      决定扫描与 digest 是否成立）与 `repo_tree_complete`（是否枚举了整个仓，只有它能支撑
      "该仓共有多少 SKILL.md" 这类论断）。子树遍历只置前者。
